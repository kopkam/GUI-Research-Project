...
MAX_SAMPLES = 200  # Use only 200 samples from all available
...
def prepare_yolo_dataset():
    ...
    # Randomly select MAX_SAMPLES if we have more
    if len(annotation_files) > MAX_SAMPLES:
        print(f"Randomly selecting {MAX_SAMPLES} samples from {len(annotation_files)} available...")
        random.shuffle(annotation_files)
        annotation_files = annotation_files[:MAX_SAMPLES]
        print(f"Selected {len(annotation_files)} files for training")
